# 桌面拆解数据集标注工具（Teardown Annotator，TDA）设计文档 v1.3
...
3. **扫描仪俯视为主视角**（身份、状态、差分在它上面判定），其他三个视角（OAK1、OAK2、RealSense）先继承身份、由模型迁移，再人工精修或核验。
...
- **跨视角（扫描仪 → 其他三视角）**：每台机器每个视角每个位姿段人工点 4 个机箱角点，估计到扫描仪的单应；**只传递实例中心点/框作为 SAM 提示与身份候选，不传递掩码**。
...
- **四视角同一身份**：第 k 步变化的实例在所有视角是同一物体；其他视角用跨视角单应投影扫描仪实例中心作为提示。
...
| S3 OAK1 迁移 + 精修（gold） | 语义角点 → 单应中心点提示 → 模型/SAM 初稿 → 审阅确认 | 0.6–1 h |
| S4 OAK2 迁移 + 核验（silver） | 同上；另有堆放区 ROI：起点帧画出全部已拆零件的框，倒序每步少一个框 | 0.4–0.6 h |
| S5 RealSense 预标 + 帧级验收（bronze） | 平面小零件用单应中心点提示、大件用模型预测；只做"这一帧整体是否可接受"的二值验收，不逐实例确认 | 10–20 min |
...
- **视角冷启动**：每个新视角先人工完成 3–4 台（用单应中心点 + SAM），加入训练集后再对该视角批量预标（跨视角域差大）。
