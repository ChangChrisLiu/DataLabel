...
subcommand. The work itself lives in :mod:`tda.pipeline`.
...
parsed arguments and returns the process exit code.
"""
...
def _open(args: argparse.Namespace) -> tuple[dict, Db]:
    """Load ``paths.yaml`` and open the database it (or ``--db``) names."""
    paths = P.load_paths(args.paths)
    return paths, P.open_db(paths, args.db)
...
# import-logs
# --------------------------------------------------------------------------- #
def logs_report(run: P.LogsRun, expected: Optional[dict[int, int]] = None) -> str:
...
def cmd_import_logs(args: argparse.Namespace) -> int:
    ...
    try:
        ...
        print(f"[import-logs] {len(imported)} desktops imported, {len(run.skipped)} skipped; "
              f"{sum(r.steps for r in imported)} steps, {sum(r.actions for r in imported)} "
              f"actions, {sum(r.instances for r in imported)} instances, "
        ...
        with open(report, "w", encoding="utf-8") as fh:
            fh.write(logs_report(run, P.expected_steps(directory)))
...
def _add_import_logs(sub) -> None:
    ...
    p.add_argument("--force", action="store_true",
                   help="re-import desktops that already have steps (drops manual edits)")
...
def cmd_backup(args: argparse.Namespace) -> int:
    ...
    paths, db = _open(args)
    try:
        dest = args.dest or P.require(paths, "backup_dir")
        out = db.backup(dest)
...
def _add_backup(sub) -> None:
    ...
    p.add_argument("--dest", default=None, help="backup folder (default paths.yaml backup_dir)")
...
def format_desktop(db: Db, row: dict) -> str:
    ...
    types = ", ".join(f"{k} {n}" for k, n in P.step_type_counts(db, row["desktop"]).items())
    lines = [
    ...
    segments = P.pose_segments(db, row["desktop"])
    for view in VIEWS:
        spans = segments.get(view) or []
        rendered = ", ".join(
            f"#{s['seg']} [{s['start']}-{s['end']}] ref {s['ref']}"
            f"{' corners' if s['corners'] else ''}" for s in spans
        )
...
def main(argv: Optional[list[str]] = None) -> int:
    ...
    except ValueError as exc:  # bad --desktops spec, bad paths.yaml
